...
<main class="max-w-7xl mx-auto w-full px-4 md:px-6 py-6 flex-1 pb-[100px] md:pb-10">
    ...
    <form id="formKonselingOnline" method="POST" action="{{ route('bk.store-form-konseling-online') }}" class="flex flex-col gap-5 animate-[fadeIn_0.5s_ease-out]">
        ...
        <div class="border-[2px] border-[#1a9488] rounded-2xl px-5 py-4 bg-white transition-all mt-2 shadow-sm">
            <label class="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" name="has_next_meeting" id="hasNextMeetingOnline" value="1" class="w-5 h-5 accent-[#1a9488]" onchange="toggleNextMeetingOnline()">
                <span class="text-[0.95rem] font-bold text-[#1a9488]">Jadwalkan Pertemuan Selanjutnya?</span>
            </label>
            
            <div id="nextMeetingFieldsOnline" class="hidden mt-4 pt-4 border-t border-[#eaeaea]">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="text-[0.75rem] font-bold text-[#1a9488] block mb-1 uppercase tracking-wider">Tanggal Pertemuan</label>
                    ...
                    <div>
                        <label class="text-[0.75rem] font-bold text-[#1a9488] block mb-1 uppercase tracking-wider">Waktu Pertemuan</label>
    ...
    <script>
        function toggleNextMeetingOnline() {
            const isChecked = document.getElementById('hasNextMeetingOnline').checked;
            const fields = document.getElementById('nextMeetingFieldsOnline');
    ...
    <div class="flex justify-end mt-8">
        <button
            ...
            class="w-full md:w-auto px-12 py-4 bg-[#1a9488] text-white rounded-full text-[1rem] font-bold shadow-[0_4px_16px_rgba(26,148,136,0.3)] hover:brightness-105 hover:-translate-y-0.5 transition-all active:scale-95 border-none cursor-pointer"
        >
            Simpan & Buat Laporan
        </button>
    </div>
