...
<main class="w-full px-4 md:px-6 py-8 flex-1 flex flex-col gap-6 max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-3 mb-2">
        ...
        <div>
            <h2 class="text-[1.3rem] md:text-[1.5rem] font-extrabold text-[#1a1a1a]">Tulis Artikel Baru</h2>
            <p class="text-sm text-[#888] mt-0.5">Buat konten edukasi yang informatif untuk para siswa</p>
        </div>
    </div>
    ...
    <form action="{{ route('bk.artikel.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-5" id="artikelForm">
        @csrf
        ...
        <div>
            ...
            <div class="border-[2px] border-[#1a9488] rounded-2xl px-5 py-3.5 bg-white focus-within:shadow-[0_0_0_3px_rgba(26,148,136,0.15)] transition-all">
                <input type="text" name="judul" value="{{ old('judul') }}" required placeholder="Ketik judul yang menarik..."
        ...
        {{-- Cover Image --}}
        <div>
            <label class="block text-[0.8rem] text-[#555] font-bold mb-2 uppercase tracking-wider">Gambar Cover <span class="text-red-400">*</span></label>
            <div class="border-[2px] border-[#1a9488] rounded-2xl px-5 py-4 bg-white transition-all">
                ...
                <label for="gambarInput" class="flex items-center gap-3 cursor-pointer group">
                    ...
                    <div>
                        <div class="text-sm font-semibold text-[#1a9488] group-hover:text-[#157a70]">Pilih Gambar Cover</div>
                        <div class="text-xs text-[#aaa]">JPG, PNG · Maks. 2MB</div>
                    </div>
                </label>
                <input id="gambarInput" type="file" name="gambar" accept="image/*" required class="hidden" onchange="previewImage(this)">
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="konten" id="konten-hidden" class="hidden">{{ old('konten') }}</textarea>
        </div>
    ...
    <div id="confirmModal" class="hidden fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-[320px] md:w-[420px] p-8 md:p-10 flex flex-col items-center gap-6 z-10 border-[2px] border-[#1a9488]">
            ...
            <p class="text-[1.1rem] md:text-[1.2rem] font-semibold text-[#1a1a1a] text-center">Apakah anda yakin menerbitkan artikel ini?</p>
            <div class="flex gap-5 w-full justify-center mt-2">
                <button type="button" onclick="document.getElementById('artikelForm').submit()" class="h-[48px] w-[90px] bg-[#1a9488] text-white rounded-full flex items-center justify-center hover:brightness-105 transition-all shadow-[0_4px_12px_rgba(26,148,136,0.3)] border-none cursor-pointer">
...
<script>
    ...
    const oldKonten = document.getElementById('konten-hidden').value;
    if (oldKonten) {
        quill.root.innerHTML = oldKonten;
    }
    ...
    function validateAndShowModal() {
        ...
        document.getElementById('konten-hidden').value = konten;
