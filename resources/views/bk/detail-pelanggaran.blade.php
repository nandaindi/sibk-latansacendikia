                ...
                <div class="bg-white border-[2px] border-[#1a9488] rounded-2xl px-5 py-5 shadow-sm flex flex-col gap-4">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="font-bold text-[1rem] text-[#1a1a1a]">{{ $pelanggaran->user->name }}</div>
                            <div class="text-[0.82rem] text-[#888]">NIS: {{ $pelanggaran->user->nis ?? '-' }} · {{ $pelanggaran->user->kelas }} {{ $pelanggaran->user->jurusan }}</div>
                        </div>
                        <div class="shrink-0">
                    ...
                    {{-- Jadwal --}}
                    <div>
                        <label class="text-[0.7rem] font-bold text-[#888] uppercase tracking-wider block mb-1">Jadwal Pertemuan</label>
                        <div class="text-[0.92rem] font-semibold text-[#1a1a1a]">
                            {{ \Carbon\Carbon::parse($pelanggaran->tanggal)->translatedFormat('l, d F Y') }} · Pukul {{ \Carbon\Carbon::parse($pelanggaran->waktu)->format('H:i') }} WIB
                        </div>
                    </div>
            ...
            <div class="lg:col-span-2 lg:sticky lg:top-4">
                ...
                <div class="bg-white border-[2px] border-[#1a9488] rounded-2xl px-5 py-5 shadow-sm">
                    ...
                    <form action="{{ route('bk.panggil-siswa.update', $pelanggaran->id) }}" method="POST" class="flex flex-col gap-5">
                        ...
                        <div>
                            <label class="text-[0.7rem] font-bold text-[#888] uppercase tracking-wider block mb-2">Hasil Pertemuan</label>
                        ...
                        <div>
                            <label class="text-[0.7rem] font-bold text-[#888] uppercase tracking-wider block mb-2">Rencana Tindak Lanjut</label>
