...
<nav class="md:hidden fixed bottom-0 left-0 w-full bg-white h-[70px] flex items-center justify-around border-t border-[#e0e0e0] z-50 pb-[env(safe-area-inset-bottom)] rounded-t-[20px] shadow-[0_-4px_20px_rgba(0,0,0,0.03)]">
    ...
    <a href="{{ route('siswa.profile.edit') }}" class="bg-transparent border-none flex flex-col items-center gap-1 p-2 {{ request()->routeIs('siswa.profile.edit') ? 'text-[#1a9488]' : 'text-[#888]' }} hover:text-[#1a9488] transition-colors no-underline">
        <svg width="22" height="22" viewBox="0 0 24 24" fill="{{ request()->routeIs('siswa.profile.edit') ? 'currentColor' : 'none' }}" stroke="currentColor" stroke-width="{{ request()->routeIs('siswa.profile.edit') ? '0.5' : '2' }}" stroke-linecap="round" stroke-linejoin="round">
            <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>
        </svg>
        <span class="text-[0.75rem] font-semibold currentColor">Profil</span>
    </a>
</nav>
