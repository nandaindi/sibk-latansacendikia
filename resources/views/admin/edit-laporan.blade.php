...
<div class="w-full">
    ...
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-[1.3rem] md:text-[1.5rem] font-extrabold text-[#1a1a1a]">Edit Laporan</h2>
        <div class="hidden md:block text-[0.95rem] text-[#888] font-medium lowercase">
            kelola laporan/detail/edit
        </div>
    </div>

    <form id="editLaporanForm" method="POST" action="{{ route('admin.kelola-laporan.edit.update') }}" class="flex flex-col gap-4">
        @csrf
        @method('PUT')
        ...
        <div class="border-[2px] border-[#1a9488] rounded-2xl px-5 py-3.5 bg-white focus-within:shadow-[0_0_0_3px_rgba(26,148,136,0.15)] transition-all">
            <input type="text" name="nama_laporan" placeholder="Nama Laporan" value="Laporan 1" required
                   class="w-full border-none outline-none text-[1rem] text-[#1a1a1a] placeholder-[#aaa] bg-transparent font-medium"/>
        </div>

        {{-- Autor --}}
        <div class="border-[2px] border-[#1a9488] rounded-2xl px-5 py-3.5 bg-white focus-within:shadow-[0_0_0_3px_rgba(26,148,136,0.15)] transition-all">
            <input type="text" name="autor" placeholder="Autor" value="Ibu Eni Kustiyorini S.Psi" required
                   class="w-full border-none outline-none text-[1rem] text-[#1a1a1a] placeholder-[#aaa] bg-transparent font-medium"/>
        </div>
        ...
        <div class="border-[2px] border-[#1a9488] rounded-2xl px-5 py-3.5 bg-white focus-within:shadow-[0_0_0_3px_rgba(26,148,136,0.15)] transition-all relative">
            <input type="date" name="date" placeholder="Date" value="2025-07-02" required
                   class="w-full border-none outline-none text-[1rem] text-[#1a1a1a] placeholder-[#aaa] bg-transparent font-medium
        ...
        {{-- Search Key --}}
        <div class="border-[2px] border-[#1a9488] rounded-2xl px-5 py-3.5 bg-white focus-within:shadow-[0_0_0_3px_rgba(26,148,136,0.15)] transition-all">
            <input type="text" name="search_key" placeholder="Search Key" value="Minggu, 2 Juli 2025" required
                   class="w-full border-none outline-none text-[1rem] text-[#1a1a1a] placeholder-[#aaa] bg-transparent font-medium"/>
        </div>

        {{-- Save Button (Triggers Edit Modal) --}}
        <div class="mt-4">
            <button type="button" onclick="showConfirmEditLaporanModal()"
...
{{-- ===== KONFIRMASI EDIT LAPORAN MODAL ===== --}}
<div id="confirmEditLaporanModal" class="hidden fixed inset-0 z-50 flex items-center justify-center">
    {{-- Backdrop --}}
    <div class="absolute inset-0 bg-black/30 backdrop-blur-sm" onclick="hideConfirmEditLaporanModal()"></div>
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-[320px] md:w-[420px] p-8 md:p-10 flex flex-col items-center gap-6 z-10 border-[2px] border-[#1a9488]">
        ...
        <p class="text-[1.1rem] md:text-[1.2rem] font-semibold text-[#1a1a1a] text-center">Apakah anda yakin edit laporan?</p>

        <div class="flex gap-5 w-full justify-center mt-2">
            {{-- Button OK (Submit Edit Form) --}}
            <button type="button" onclick="submitEditLaporanForm()"
            ...
            {{-- Button Cancel --}}
            <button type="button" onclick="hideConfirmEditLaporanModal()"
...
@push('scripts')
<script>
function showConfirmEditLaporanModal() {
    const form = document.getElementById('editLaporanForm');
    if (form.checkValidity()) {
        document.getElementById('confirmEditLaporanModal').classList.remove('hidden');
        document.getElementById('confirmEditLaporanModal').classList.add('flex');
    } else {
        form.reportValidity();
    }
}

function hideConfirmEditLaporanModal() {
    document.getElementById('confirmEditLaporanModal').classList.add('hidden');
    document.getElementById('confirmEditLaporanModal').classList.remove('flex');
}

function submitEditLaporanForm() {
    document.getElementById('editLaporanForm').submit();
}
</script>
@endpush
