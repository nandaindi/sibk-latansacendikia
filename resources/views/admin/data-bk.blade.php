...
<div class="flex items-start justify-between mb-5 gap-4 flex-wrap">
    ...
    <div class="flex flex-col items-end gap-1 w-full sm:w-auto">
        <span class="text-[0.78rem] text-[#888] mr-1 hidden sm:block">data guru bk</span>
        <div class="flex items-center border-[2px] border-[#1a9488] rounded-full px-4 py-2 bg-white gap-2 w-full sm:w-52 focus-within:shadow-[0_0_0_3px_rgba(26,148,136,0.15)] transition-all">
            ...
            <input type="text" id="searchInput" placeholder="Search" onkeyup="filterBk()"
...
<div class="hidden sm:flex items-center gap-4 bg-[#1a9488] text-white rounded-2xl px-5 py-3 mb-1 font-bold text-[0.85rem] tracking-wide uppercase">
    ...
    <span class="flex-1 hidden md:block">NIP</span>
    <span class="flex-1 hidden lg:block">Jabatan</span>
    <span class="flex-1 hidden sm:block">Telepon</span>
    <span class="w-[112px] shrink-0 text-center">Aksi</span>
</div>

{{-- Data List --}}
<div id="bkList" class="flex flex-col gap-3 w-full">
    @forelse($bk as $item)
    <div class="bk-item bg-white border-[2px] border-[#1a9488] rounded-2xl px-4 sm:px-5 py-3.5 flex items-center gap-3 sm:gap-4 shadow-sm hover:shadow-md transition-shadow">
        <span class="w-[30px] sm:w-[40px] shrink-0 text-center text-[0.85rem] font-bold text-[#1a9488]">{{ $loop->iteration + ($bk->currentPage() - 1) * $bk->perPage() }}</span>
        <span class="flex-1 text-[0.93rem] font-semibold text-[#1a1a1a] min-w-[100px] truncate">{{ $item->name }}</span>
        <span class="flex-1 text-[0.9rem] text-[#555] hidden md:block">{{ $item->nip ?? '-' }}</span>
        <span class="flex-1 text-[0.9rem] text-[#555] hidden lg:block">{{ $item->jabatan ?? '-' }}</span>
        <span class="flex-1 text-[0.9rem] text-[#555] hidden sm:block">{{ $item->telepon ?? '-' }}</span>
        <div class="flex items-center justify-center gap-1.5 sm:gap-2 shrink-0 w-[100px] sm:w-[112px]">
            <a href="{{ route('admin.data-bk.detail', ['id' => $item->id]) }}" title="Detail" class="w-8 h-8 rounded-full bg-[#e6f4f2] text-[#1a9488] flex items-center justify-center hover:bg-[#1a9488] hover:text-white transition-colors">
            ...
            <a href="{{ route('admin.data-bk.edit', ['id' => $item->id]) }}" title="Edit" class="w-8 h-8 rounded-full bg-[#fff4e5] text-[#f59e0b] flex items-center justify-center hover:bg-[#f59e0b] hover:text-white transition-colors">
            ...
            <button type="button" onclick="showDeleteBkModal({{ $item->id }})" title="Hapus" class="w-8 h-8 rounded-full bg-[#fce8e8] text-[#ef4444] flex items-center justify-center hover:bg-[#ef4444] hover:text-white transition-colors border-none cursor-pointer">
    ...
    @empty
    <div class="text-center py-6 text-gray-500 font-medium bg-white rounded-2xl border-[2px] border-[#edf2f1]">Belum ada data guru BK.</div>
    @endforelse

    {{-- Pagination --}}
    <div class="mt-4">
        {{ $bk->appends(request()->query())->links() }}
    </div>
</div>
