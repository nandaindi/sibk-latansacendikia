...
<div class="w-full">
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <h2 class="text-[1.2rem] font-extrabold text-[#1a1a1a]">Edit Data Siswa</h2>
        </div>
        <div class="hidden md:block text-[0.95rem] text-[#888] font-medium">
            Kelola Data / Data Siswa / Edit
        </div>
    </div>

    <form id="editSiswaForm" method="POST" action="{{ route('admin.data-siswa.update', ['id' => $user->id]) }}" class="flex flex-col gap-4">
        @csrf
        @method('PUT')
        ...
        <div class="mb-2">
            ...
            <div class="border-[2px] border-[#1a9488] rounded-2xl px-5 py-4 bg-white focus-within:shadow-[0_0_0_3px_rgba(26,148,136,0.15)] transition-all mb-3">
                ...
                <input type="text" name="username" placeholder="Username untuk login" value="{{ old('username', $user->username) }}" required pattern="[A-Za-z0-9\.]+" title="Hanya kombinasi huruf, angka, dan titik" oninput="this.value = this.value.replace(/[^a-zA-Z0-9\.]/g, '')"
                       class="w-full border-none outline-none text-[1rem] text-[#1a1a1a] bg-transparent font-medium"/>
            </div>
...
<div id="confirmEditModal" class="hidden fixed inset-0 z-50 flex items-center justify-center">
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-[320px] md:w-[420px] p-8 md:p-10 flex flex-col items-center gap-6 z-10 border-[2px] border-[#1a9488]">
        ...
        <p class="text-[1.1rem] md:text-[1.2rem] font-semibold text-[#1a1a1a] text-center">Apakah anda yakin edit data siswa?</p>
