...
@section('title', 'Detail Data Siswa – Admin')
...
<div class="w-full">
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <h2 class="text-[1.3rem] md:text-[1.5rem] font-extrabold text-[#1a1a1a]">Detail Data Siswa</h2>
        </div>
        <div class="hidden md:block text-[0.95rem] text-[#888] font-medium">
            Kelola Data / Data Siswa / Detail
        </div>
    </div>
    ...
    <div class="bg-white border-[2px] border-[#1a9488] rounded-2xl p-6 md:p-8 shadow-sm mb-6">
        <div class="flex items-center gap-6 mb-6">
            ...
            <div>
                ...
                <p class="text-[0.9rem] text-[#888] font-medium">NIS: {{ $user->nis ?? '-' }}</p>
                <p class="text-[0.85rem] text-[#1a9488] font-bold uppercase mt-1">Siswa</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="bg-[#f8fffe] rounded-xl p-4 border border-[#e0f5f3]">
                <p class="text-[0.75rem] font-bold text-[#1a9488] uppercase tracking-wide mb-1">Kelas / Rombel</p>
                <p class="text-[0.95rem] font-semibold text-[#1a1a1a]">{{ $user->kelas ?? '-' }}</p>
            </div>
            <div class="bg-[#f8fffe] rounded-xl p-4 border border-[#e0f5f3]">
                <p class="text-[0.75rem] font-bold text-[#1a9488] uppercase tracking-wide mb-1">Jurusan</p>
                <p class="text-[0.95rem] font-semibold text-[#1a1a1a]">{{ $user->jurusan ?? '-' }}</p>
            </div>
            <div class="bg-[#f8fffe] rounded-xl p-4 border border-[#e0f5f3]">
            ...
            <div class="bg-[#f8fffe] rounded-xl p-4 border border-[#e0f5f3]">
                <p class="text-[0.75rem] font-bold text-[#1a9488] uppercase tracking-wide mb-1">Tempat, Tanggal Lahir</p>
                <p class="text-[0.95rem] font-semibold text-[#1a1a1a]">{{ $user->tempat_lahir ?? '-' }}{{ $user->tanggal_lahir ? ', ' . \Carbon\Carbon::parse($user->tanggal_lahir)->format('d F Y') : '' }}</p>
            </div>
            ...
            <div class="bg-[#f8fffe] rounded-xl p-4 border border-[#e0f5f3]">
                <p class="text-[0.75rem] font-bold text-[#1a9488] uppercase tracking-wide mb-1">No. Telepon Siswa</p>
        ...
        <div class="flex gap-3 mt-6">
            <a href="{{ route('admin.data-siswa.edit', ['id' => $user->id]) }}" class="px-6 py-2 bg-[#f59e0b] text-white rounded-full text-[0.85rem] font-bold hover:brightness-105 transition-all no-underline shadow-md">Edit Data</a>
            <button onclick="showDeleteModal({{ $user->id }})" class="px-6 py-2 bg-[#ef4444] text-white rounded-full text-[0.85rem] font-bold hover:brightness-105 transition-all border-none cursor-pointer shadow-md">Hapus Data</button>
        </div>
    </div>
...
<div id="deleteModal" class="hidden fixed inset-0 z-50 flex items-center justify-center">
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-[320px] md:w-[420px] p-8 md:p-10 flex flex-col items-center gap-6 z-10 border-[2px] border-[#1a9488]">
        ...
        <div class="flex gap-5 w-full justify-center mt-2">
            <form id="deleteForm" action="{{ route('admin.data-siswa.destroy', ['id' => $user->id]) }}" method="POST">
                @csrf
                @method('DELETE')
