...
<div class="bg-white rounded-[24px] border border-[#edf2f1] shadow-[0_4px_12px_rgba(0,0,0,0.02)] overflow-x-hidden w-full">
    <table id="siswaTable" class="w-full text-left border-collapse display">
        <thead>
            <tr class="bg-[#f8fcfb] border-b border-[#edf2f1]">
                ...
                <th class="p-4 text-[0.85rem] text-[#888] font-bold uppercase tracking-wider">NISN</th>
                <th class="p-4 text-[0.85rem] text-[#888] font-bold uppercase tracking-wider">Kelas</th>
                <th class="p-4 text-[0.85rem] text-[#888] font-bold uppercase tracking-wider">Jenis Kelamin</th>
        ...
        <tbody class="divide-y divide-[#edf2f1]">
            ...
            <tr class="hover:bg-[#fcfdfd] transition-colors">
                ...
                <td class="p-4 text-[0.9rem] text-[#555]">{{ $item->nis ?? '-' }}</td>
                <td class="p-4 text-[0.9rem] text-[#555]">{{ $item->kelas ?? '-' }}</td>
                <td class="p-4 text-[0.9rem] text-[#555]">{{ $item->jenis_kelamin ?? '-' }}</td>
                <td class="p-4 text-center">
                    <div class="flex items-center justify-center gap-2">
                        <a href="{{ route('admin.data-siswa.detail', ['id' => $item->id]) }}" title="Detail" class="p-2 text-[#1a9488] hover:bg-[#e0f5f3] rounded-lg transition-colors">
                        ...
                        <a href="{{ route('admin.data-siswa.edit', ['id' => $item->id]) }}" title="Edit" class="p-2 text-[#f59e0b] hover:bg-[#fffbeb] rounded-lg transition-colors">
                        ...
                        <button type="button" onclick="showDeleteSiswaModal({{ $item->id }})" title="Hapus" class="p-2 text-[#ef4444] hover:bg-[#fef2f2] rounded-lg transition-colors border-none bg-transparent cursor-pointer">
...
$(document).ready(function() {
    $('#siswaTable').DataTable({
        language: { searchPlaceholder: "Cari siswa..." },
        columnDefs: [
